...
int ClientCharacterControlGame::initGame()
{
	...
	//std::cout << "-----------------------------------------Init game client" << endl;
	...
	// create the GameObjectmanager addon that is in charge of game objects in this demo
	{
		...
		SceneNode* pMainEnemySN = new(hSN) SceneNode(*m_pContext, m_arena, hSN);
		pMainEnemySN->addDefaultComponents();

		pMainEnemySN->m_base.setPos(Vector3(-10.0f, 0, 10.0f));
		...
		//float m_minX = pImrodMeshInst->m_minX;
		//std::cout << "THIS IS WITHIN CLIENT CHARACTER CONTROL GAME CPP!!!!!!!!" << std::endl << m_minX << std::endl;
		

		pMainEnemySN->addComponent(hImrodMeshInst);
		//pImrodMeshInst->MeshInstSceneNode = pMainEnemySN;
		My_Enemy = hSN.getObject();
...
// 			}
	...
	//m_pContext->getSoundManager()->playSound("makeroom", SoundManager::SOUND_OPTION_FADE_IN & SoundManager::SOUND_OPTION_LOOP); //plays as bg currently
	//make new bg sound manager
	
	PE::Handle hSN("SCENE_NODE", sizeof(SceneNode));
	SceneNode* pMainSN = new(hSN) SceneNode(*m_pContext, m_arena, hSN);
	pMainSN->addDefaultComponents();

	pMainSN->m_base.setPos(Vector3(-1.0f, 0.3f, 0.1f));

	SoundManager::emitter* Emitter = m_pContext->getSoundManager()->createEmitter(pMainSN, 10.0f);

	m_pContext->getSoundManager()->play3DSound("jingle", Emitter, SoundManager::SOUND_OPTION_LOOP|SoundManager::SOUND_OPTION_IMPORTANT);

	PE::Handle hImrodMeshInst = PE::Handle("MeshInstance", sizeof(MeshInstance));
	MeshInstance* pImrodMeshInst = new(hImrodMeshInst) MeshInstance(*m_pContext, m_arena, hImrodMeshInst);

	pImrodMeshInst->addDefaultComponents();
	pImrodMeshInst->initFromFile("windowplaneshape1_mesh.mesha", "Default", m_pContext->m_gameThreadThreadOwnershipMask);
    ...
    if (spawnALotOfMeshes)
    {
        for (int ix = 0; ix < maxX; ++ix)
        {
            for (int iy = 0; iy < 10; ++iy)
            {
				...
				//float m_minX2 = pImrodMeshInst->m_minX;
				//std::cout << "THIS IS WITHIN CLIENT CHARACTER CONTROL GAME CPP WHERE A LOT OF THEM ARE SPAWNED!!!!!!!!" << std::endl << m_minX2 << std::endl;
				pMainSN->addComponent(hImrodMeshInst);
	...
	{
		//snow
		m_pContext->getParticleEmitter()->emitParticles(150, "rainahh-01.dds", 0.01f, m_pContext->m_gameThreadThreadOwnershipMask,2.5f, Vector3(0.0f, 0.0f, 0.0f), 0.01f, 20.0f,0.7f,2.0f);

		RootSceneNode::Instance()->addComponent(m_pContext->getParticleEmitter());
