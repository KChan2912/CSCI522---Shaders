...
void EffectManager::buildFullScreenBoard()
{
	...
	m_hRecolorEffect = getEffectHandle("celrecolor.fx");
	m_hOutlineEffect = getEffectHandle("sobeloutline.fx");
...
{
	//DO NOT LOSE THIS (seems to affect the whole screen directly?)
...
void EffectManager::CelRecolor()
{
	//DO NOT LOSE THIS (seems to affect the whole screen directly?)
	Effect& curEffect = *m_hRecolorEffect.getObject<Effect>();
	if (!curEffect.m_isReady)
		return;
...
{
	//DO NOT LOSE THIS (seems to affect the whole screen directly?)
	Effect& curEffect = *m_hOutlineEffect.getObject<Effect>();
	if (!curEffect.m_isReady)
		return;
...
void EffectManager::debugDrawRenderTarget(bool drawGlowRenderTarget, bool drawSeparatedGlow, bool drawGlow1stPass, bool drawGlow2ndPass, bool drawShadowRenderTarget)
{
	...
	Effect& curEffect = *m_hMotionBlurEffect.getObject<Effect>();//*m_hMotionBlurEffect.getObject<Effect>();
	if (!curEffect.m_isReady)
		return;
